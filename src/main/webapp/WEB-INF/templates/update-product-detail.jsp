...
<div class="input-group mb-3">
    ...
    <c:forEach var="item" items="${danhSach.sanPham}">
        <option value="${item.id}" ${item.id==detailUpdate.sanPham.id?'selected':''}> ${item.tenSanPham} </option>
    </c:forEach>
</select><br>
    ...
    <div class="input-group mb-3">
        ...
        <c:forEach var="item" items="${danhSach.mauSac}">
            <option value="${item.id}" ${item.id==detailUpdate.mauSac.id?'selected':''}> ${item.tenMau} </option>
        </c:forEach>
    </select><br>
        ...
        <div class="input-group mb-3">
            ...
            <c:forEach var="item" items="${danhSach.kichCo}">
                <option value="${item.id}" ${item.id==detailUpdate.kichCo.id?'selected':''}> ${item.size} </option>
            </c:forEach>
        </select><br>
            ...
            <div class="input-group mb-3">
                ...
                <c:forEach var="item" items="${danhSach.chatLieu}">
                    <option value="${item.id}" ${item.id==detailUpdate.chatLieu.id?'selected':''}> ${item.tenChatLieu} </option>
                </c:forEach>
            </select><br>
                ...
                <div class="input-group mb-3">
                    ...
                    <c:forEach var="item" items="${danhSach.deGiay}">
                        <option value="${item.id}" ${item.id==detailUpdate.deGiay.id?'selected':''}> ${item.loaiDe} </option>
                    </c:forEach>
                </select><br>
                    ...
                    <table class="table">
                        ...
                        <c:forEach items="${product.content}" var="pro">
                            <tr>
                                ...
                                <td><a class="btn btn-primary" href="/product-detail/get/${pro.id}" role="button">Hien thi</a>
                                </td>
